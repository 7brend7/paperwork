angular.module('paperworkNotes').controller('SidebarNotebooksController',
  function ($scope, $rootScope, $location, $routeParams, $filter, $q, NotebooksService, NotesService, ngDraggable) {
    ...
    $scope.toggleCalendarView = function () {
      if ($('.sidebar .calendar-ul-container').hasClass('expanded')) {
        $('.sidebar .calendar-ul-container').removeClass('expanded').slideUp();
      }
      else {
        $('.sidebar .calendar-ul-container').addClass('expanded').slideDown();
      }
